...
public class UserInputService {
  ...
  @Inject
  public UserInputService(ResourceLoader resourceLoader) {
    try {
  ...
  public void validateComment(String input) {
    if (Strings.isBlank(input)) {
      throw new SoftException("Comment cannot be empty");
    }
    // check min length
    if (input.length() < 20) {
      throw new SoftException("Comment must be at least 20 characters long");
    }
    if (input.length() > 1000) {
      throw new SoftException("Comment cannot be longer than 1000 characters");
    }
    checkProfanity(input);
  }

  public void checkProfanity(String input) {
